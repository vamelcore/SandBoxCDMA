...
<td height="30px" style="border-bottom:1px solid #c6d5e1;"><table cellpadding="0" cellspacing="5" border="0"><tr><td><?php if($_SESSION['add_priv_pro'] == 1) {printf("<input name=\"add\" type=\"button\" value=\"Добавить продажу\" onclick=\"top.location.href='update/add_prodaja.php'\">");} ?> </td><td><input name="update" type="button" value="Сортировка по дате" onclick="top.location.href='prodaja.php'"></td> <?php if (isset($_SESSION['rollpriv']) && ($_SESSION['rollpriv'] == 1)) { printf("<td><input type=\"button\" value=\"Отмена продажи\" onclick=\"top.location.href='update/rollback_prodaja.php'\"></td>"); } ?> <td><p style="font-size:10pt;">Таблица: "Продажи". </p></td><td><p style="font-size:10pt;">Дата:</p></td><td><form action="" method="post"><select name="pr_sec_data" onchange="javascript:form.submit()"><option value="All">Все</option>
	...
	while ($myrow_date = mysql_fetch_array($result_date));
		...
		?></select></form></td><td><form action="prodaja_to_xls.php"><input type="submit" value="Сохранить в XLS"></form></td><td><p style="font-size:10pt;">Сумма на счету в Intertelecom:</p></td><td><p style="font-size:10pt; color: green;">
			<?php 
	$res_rah = mysql_query("SELECT schet FROM operatoru WHERE oper = 'Intertelecom'",$db);
	$myr_rah = mysql_fetch_array($res_rah);
	echo $myr_rah['schet']; 
	        ?></p></td><td><p style="font-size:10pt;">грн.</p></td><td><p style="font-size:10pt;">Сумма в кассе:</p></td><td><p style="font-size:10pt; color: green;"><?php 
	...
	<table cellpadding="0" cellspacing="0" border="0" id="table" class="sortable">
		<thead>
			<tr>
                ...
                <th><h3>Серийный номер</h3></th>
				<th><h3>Штрих-код</h3></th>
				<th><h3>Вознаграждение за оборуд, грн</h3></th>
				<th><h3>Стоимость оборудов, грн</h3></th>
				<th><h3>Процент от прод, грн.</h3></th>
				<th><h3>Оператор</h3></th>
				<th><h3>Тарифный план</h3></th>
				<th><h3>Вознаграждение за ТП, грн</h3></th>				
				<th><h3>Оплата ТП, подключение, грн</h3></th>
...
if (isset($id_kategory_from_array)) {
	$res_tovar = mysql_query("SELECT `ID` FROM `prase` WHERE `ID_kategorii` = '$id_kategory_from_array' AND `tovar` = '{$myrow['naimenovanie']}'",$db); 
...
printf("<td><p %s>%s</p></td><td><p %s>%s %s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td><td><p %s>%s</p></td>",$style,$myrow['kategoria'],$style,$myrow['naimenovanie'],$div_with_tov,$style,$myrow['serialnum'],$style,$myrow['shtrihkod'],$style,$myrow['voznag_za_jelezo'],$style,$myrow['stoimost'],$style,$myrow['procent_prod'],$style,$myrow['operator'],$style,$myrow['tarifn_plan'],$style,$myrow['voznag_za_tp'],$style,$myrow['oplata_tp_podkluchenie'],$style,$myrow['kluch_evdo'],$style,$myrow['kontakt_nomer_tel'],$style,$myrow['FIO'],$style,$myrow['abonent_nomer'],$style,$myrow['mesto_polz'],$style,$myrow['skidka']);

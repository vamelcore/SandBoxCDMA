...
printf("</table></td><td valign=top align=center><table bgcolor=#c6d5e1 border=0 cellpadding=2 cellspacing=1 width=180><tr><td colspan='2' bgcolor=#c1cedb align='center'><strong>Суммарная стоемость оборудования, грн</strong></td></tr>");
$no = 0;
do {
$no = $no+1;
...
<tr bgcolor="#dce6ee">
  	<td><p>ПРОДАНО, ВСЕГО</p></td>
  	<?php
  	if ($_SESSION['id_mag_selected'] <> 'all') {
  		$res_summ_prodaj = mysql_query("SELECT magazin, COUNT(ID) FROM prodaja WHERE b != 'П' AND magazin = '{$myr_mag['name']}' AND sec_data = '$date_val' GROUP BY magazin",$db);
	...
	printf("<td><p align=\"center\"><strong>%s</strong></p></td><td><p align=\"center\"></p></td>",$summa_prodaj['COUNT(ID)']);}
		...
		$i = 1; $summa = 0;
		do {						
		$res_summ_prodaj = mysql_query("SELECT magazin, COUNT(ID) FROM prodaja WHERE b != 'П' AND magazin = '{$_SESSION['name_mag'][$i]}' AND sec_data = '$date_val' GROUP BY magazin",$db);	
		$summa_prodaj = mysql_fetch_array($res_summ_prodaj);				
		if ($summa_prodaj['COUNT(ID)'] == '') {$summa_prodaj['COUNT(ID)'] = 0;}
  ...
  <tr bgcolor="#ecf2f6">
    <td><p>Терминалы:</p></td>
  	<?php
  	if ($_SESSION['id_mag_selected'] <> 'all') { 
  		$res_summ_prodaj_t = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'Т' AND magazin = '{$myr_mag['name']}' AND sec_data = '$date_val' GROUP BY b",$db);
	...
	printf("<td><p align=\"center\"><strong>%s</strong></p></td><td><p align=\"center\">%s</p></td>",$summa_prodaj_t['COUNT(ID)'],$plan_term);}
		...
		$i = 1; $summa = 0;
		do {
		$res_summ_prodaj_t = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'Т' AND magazin = '{$_SESSION['name_mag'][$i]}' AND sec_data = '$date_val' GROUP BY b",$db);	
		$summa_prodaj_t = mysql_fetch_array($res_summ_prodaj_t);
	    if ($summa_prodaj_t['COUNT(ID)'] == '') {$summa_prodaj_t['COUNT(ID)'] = 0;}
  ...
  <tr bgcolor="#ecf2f6">
	<td><p>Аксесуары:</p></td>
  	<?php
  	if ($_SESSION['id_mag_selected'] <> 'all') { 
  		$res_summ_prodaj_a = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'А' AND magazin = '{$myr_mag['name']}' AND sec_data = '$date_val' GROUP BY b",$db);
	...
	printf("<td><p align=\"center\"><strong>%s</strong></p></td><td><p align=\"center\">%s</p></td>",$summa_prodaj_a['COUNT(ID)'],$plan_acses);}
		...
		$i = 1; $summa = 0;
		do {
		$res_summ_prodaj_a = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'А' AND magazin = '{$_SESSION['name_mag'][$i]}' AND sec_data = '$date_val' GROUP BY b",$db);	
		$summa_prodaj_a = mysql_fetch_array($res_summ_prodaj_a);
	    if ($summa_prodaj_a['COUNT(ID)'] == '') {$summa_prodaj_a['COUNT(ID)'] = 0;}
  ...
  <tr bgcolor="#ecf2f6">
	<td><p>Стартовые:</p></td>
  	<?php
  	if ($_SESSION['id_mag_selected'] <> 'all') { 
  		$res_summ_prodaj_s = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'С' AND magazin = '{$myr_mag['name']}' AND sec_data = '$date_val' GROUP BY b",$db);
	...
	printf("<td><p align=\"center\"><strong>%s</strong></p></td><td><p align=\"center\">%s</p></td>",$summa_prodaj_s['COUNT(ID)'],$plan_starpak);}
		...
		$i = 1; $summa = 0;
		do {
		$res_summ_prodaj_s = mysql_query("SELECT b, COUNT(ID) FROM prodaja WHERE b = 'С' AND magazin = '{$_SESSION['name_mag'][$i]}' AND sec_data = '$date_val' GROUP BY b",$db);	
		$summa_prodaj_s = mysql_fetch_array($res_summ_prodaj_s);
	    if ($summa_prodaj_s['COUNT(ID)'] == '') {$summa_prodaj_s['COUNT(ID)'] = 0;}

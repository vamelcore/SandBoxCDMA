...
$res_tov=mysql_query("SELECT ID FROM prase WHERE ID_kategorii = '{$myr_kat['ID']}' AND tovar = '{$myrow['naimenovanie']}'",$db);
...
//	    if (mysql_num_rows($res_akc) == 1) {
		if ($myrow['akciya'] == 'true') {
		       $summa_akc=$myrow['stoimost'] + $myrow['oplata_tp_podkluchenie'];
				 ...
				 else {printf("<tr><td style='border-bottom:1px solid #c6d5e1;'>Увеличение суммы на счете в <strong>%s</strong>:</td><td style='border-bottom:1px solid #c6d5e1;'><strong>%s</strong> грн<input name='Schet_op' type='hidden' value='%s'></td></tr>",$myrow['operator'],$summa_akc,$summa_akc);}
	          ...
	          if ($myr_op['priznak'] == 'Тарыфный план + Оборудование') {$myr_rahunok = $myrow['oplata_tp_podkluchenie'] + $myrow['stoimost'];}
			    if ($myrow['sposob_opl'] == 'tu') {printf("<tr><td style='border-bottom:1px solid #c6d5e1;'>Поскольку этот тарифный план был продан по безналичному расчету для юридического лица, то сумма на персональном счету оператора не меняется</td><td style='border-bottom:1px solid #c6d5e1;'><input name='Schet_op' type='hidden' value='0'></td></tr>");}
				else {printf("<tr><td style='border-bottom:1px solid #c6d5e1;'>Увеличение суммы на счете в <strong>%s</strong>:</td><td style='border-bottom:1px solid #c6d5e1;'><strong>%s</strong> грн<input name='Schet_op' type='hidden' value='%s'></td></tr>",$myrow['operator'],$myr_rahunok,$myr_rahunok);}
      ...
      if ($myrow['sposob_opl'] == 'tu') {printf("<tr><td style='border-bottom:1px solid #c6d5e1;'>Поскольку этот тарифный план был продан по безналичному расчету для юридического лица, то сумма на персональном счету оператора не меняется</td><td style='border-bottom:1px solid #c6d5e1;'><input name='Schet_op' type='hidden' value='0'></td></tr>");}
      else {printf("<tr><td style='border-bottom:1px solid #c6d5e1;'>Увеличение суммы на счете в <strong>%s</strong>:</td><td style='border-bottom:1px solid #c6d5e1;'><strong>%s</strong> грн<input name='Schet_op' type='hidden' value='%s'></td></tr>",$myrow['operator'],$myrow['oplata_tp_podkluchenie'],$myrow['oplata_tp_podkluchenie']);}

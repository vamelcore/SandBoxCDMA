...
<table width="420" cellpadding="10" cellspacing="0" style="border:1px solid #fff;">
  <tr>
    <td colspan="2" align="center" style="background:url(../images/header-bg.gif); text-align:center; color:#cfdce7; border:1px solid #fff; border-right:none; height:20px;"><b><p>Добавить продажу</p></b></td>
    </tr>
...
while($no < $_SESSION['count_mag']);
 ...
 ?>
  ...
  <tr bgcolor="#dce6ee">
    <td><p>Тип:</p></td>
  	<td><select name="b"><option title="Используется для продажи товаров из условной подкатегории аксесуары" value="А">Аксесуары</option><option title="Используется для продажи товаров из условной подкатегории терминалы" value="Т">Терминалы</option><option title="Используется для продажи стартовых пакетов" value="С">Стартовые</option><option title="Используется для продажи только тарифного плана, в этом случае не нужно задавать категорию и наименование оборудования" value="П">Подключения</option></select></td>
  </tr>
  <tr bgcolor="#ecf2f6">
...
//while ($myrow_kat = mysql_fetch_array($result_kat));
  ...
  <tr bgcolor="#dce6ee">
	<td height="20"><p>Вознаграждение за оборуд.:</p></td>
  	<td id="voznagj"></td>
    </tr>
  <tr bgcolor="#ecf2f6">
  	<td height="20"><p>Стоимость оборудования:</p></td>
  	<td id="stoim"></td>
    </tr>
  <tr bgcolor="#dce6ee">
...
$myrow_op = mysql_fetch_array($result_op);  		
 ...
 while ($myrow_op = mysql_fetch_array($result_op));  		
  ...
  <tr bgcolor="#dce6ee">
	<td height="20"><p>Тарифный план:</p></td>
  	<td><select name="tarifplan" id="tarpl"><option value=""></select></td>
    </tr>
  <tr bgcolor="#ecf2f6">
	<td height="20"><p>Вознаграждение за ТП:</p></td>
  	<td id="voznagtp"></td>
    </tr>
  <tr bgcolor="#dce6ee">
	<td height="20"><p>Оплата ТП, подключение:</p></td>
  	<td id="oplata"></td>
    </tr>
  <tr bgcolor="#ecf2f6">
	<td><p>Ключ EVDO:</p></td>
